...
Here is the algorithm that we are going to use. We start from the upper right corner of the matrix and compare its value with the key.
...
Runtime complexity #
The runtime complexity of this solution is O(m+n)O(m+n) where mm is the number of rows and nn is the number of columns.
